...
class _AcademicStructureWorkflowPageState
  ...
  Future<void> _createYear() async {
    ...
    final ok = await _formDialog('Tạo năm học', [
    ...
    try {
      ...
      _message(
        'Không thể tạo năm học. Vui lòng kiểm tra thông tin và thử lại.',
        error: true,
      );
    }
  }
  ...
  Future<void> _createClass(
    ...
    try {
      ...
      _message(
        'Không thể tạo lớp. Vui lòng kiểm tra thông tin và thử lại.',
        error: true,
      );
    }
  }

  Future<void> _assignHomeroom(
    ...
    try {
      ...
      _message(
        'Không thể gán giáo viên chủ nhiệm. Vui lòng thử lại.',
        error: true,
      );
    }
  }
        ...
        ...years.map((year) {
          ...
          return Card(
            child: ExpansionTile(
              ...
              trailing: Chip(
                label: Text(_statusLabel('${year['status'] ?? ''}')),
              ),
              children: children
                  .map(
                    (s) => ListTile(
                      ...
                      title: Text(_name(s)),
                      subtitle: Text(
                        formatViDateRange(s['startDate'], s['endDate']),
                      ),
                    ),
                  )
                  .toList(),
    ...
    children: [
      ...
      ...subjects.map(
        (e) => ListTile(
          ...
          title: Text(_name(e)),
          subtitle: Text('${e['code'] ?? ''} · Hệ số ${e['coefficient'] ?? 1}'),
        ),
      ),
      const Divider(),
      Text(
...
class _PlacementTabState extends State<_PlacementTab> {
  ...
  Map<String, dynamic> get _payload => {
    ...
    'lockedPlacements': _lockedPlacements.entries
        .map((entry) => {'studentId': entry.key, 'classCode': entry.value})
        .toList(),
  };
    ...
    return ListView(
      ...
      children: [
          ...
          children: [
            Expanded(
              child: DropdownButtonFormField<String>(
                ...
                items: ['6', '7', '8', '9', '10', '11', '12']
                    .map(
                      (e) => DropdownMenuItem(value: e, child: Text('Khối $e')),
                    )
                    .toList(),
                onChanged: (v) => setState(() {
                  _grade = v!;
            ...
            Expanded(
              child: DropdownButtonFormField<int>(
                ...
                items: [35, 40, 45, 50]
                    .map(
                      (e) => DropdownMenuItem(value: e, child: Text('$e HS')),
                    )
                    .toList(),
                onChanged: (v) => setState(() {
                  _capacity = v!;
        ...
        if (_preview != null) ...[
          const SizedBox(height: 16),
          Text(
            '${_preview!['candidateCount'] ?? 0} ứng viên · '
          ...
          ...plans.map((raw) {
            ...
            final students = (item['students'] as List? ?? const [])
                .map(
                  (rawStudent) => Map<String, dynamic>.from(rawStudent as Map),
                )
                .toList();
            return Card(
              child: ExpansionTile(
                ...
                title: Text(
                  '$classCode${item['newClass'] == true ? ' · mới' : ''}',
                ),
                subtitle: Text(
