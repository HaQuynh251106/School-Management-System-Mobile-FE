...
class _AcademicPlanProgressPageState extends State<AcademicPlanProgressPage> {
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      final values = await Future.wait(
          [_api.semesters(), _api.subjects(), _api.classes()]);
      _semesters = values[0];
      _subjects = values[1];
      _classes = values[2];
  ...
  Future<void> _editRequirement([Map<String, dynamic>? item]) async {
    ...
    final weekly =
        TextEditingController(text: '${item?['weeklyPeriods'] ?? 3}');
    ...
    final examStart =
        TextEditingController(text: '${item?['examWindowStart'] ?? ''}');
    final examEnd =
        TextEditingController(text: '${item?['examWindowEnd'] ?? ''}');
    final milestone =
        TextEditingController(text: '${item?['milestone'] ?? ''}');
    ...
    try {
      await _api.saveCurriculumRequirement({
        ...
        'examWindowStart':
            examStart.text.trim().isEmpty ? null : examStart.text.trim(),
        'examWindowEnd':
            examEnd.text.trim().isEmpty ? null : examEnd.text.trim(),
    ...
    final ok = await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: TextField(
              ...
              maxLines: 3,
              decoration:
                  const InputDecoration(labelText: 'Ghi chú cho giáo viên'),
            ),
            actions: [
              TextButton(
                  onPressed: () => Navigator.pop(context, false),
      ...
      if (status == 'APPROVED' && mounted) {
        final openSchedule = await showDialog<bool>(
              ...
              builder: (context) => AlertDialog(
                ...
                content: const Text(
                  'Backend hiện mới ghi nhận phê duyệt, chưa tự tạo một buổi học bù. '
                  'Hãy mở quản lý thời khóa biểu, điều chỉnh bản nháp và phát hành. '
          ...
          ...items.map((item) => Card(
                child: ListTile(
                  ...
                  subtitle: Text(
                      ...
                      'Thi: ${item['examWindowStart'] ?? 'chưa chốt'} → ${item['examWindowEnd'] ?? 'chưa chốt'}'
                      '${('${item['milestone'] ?? ''}').isEmpty ? '' : '\nMốc: ${item['milestone']}'}'),
                  isThreeLine: true,
                  trailing: PopupMenuButton<String>(
                    onSelected: (v) =>
                        v == 'edit' ? onEdit(item) : onDelete('${item['id']}'),
                    itemBuilder: (_) => const [
...
class _ProgressComparison extends StatelessWidget {
  ...
  int _periods(List<Map<String, dynamic>> rows) => rows.fold(
      0, (sum, row) => sum + (row['completedPeriods'] as num? ?? 0).toInt());
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      const Card(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Ba bước người dùng cần làm',
      ...
      ...grouped.entries.map((entry) {
        ...
        final dates =
            byClass.values.map(_latest).whereType<DateTime>().toList();
        ...
        final missingClasses =
            byClass.values.where((rows) => rows.isEmpty).length;
        final comparable = byClass.length >= 2;
        final balanced =
            comparable && missingClasses == 0 && dayGap <= 2 && periodGap <= 1;
        final requirement = requirements.where((item) =>
        ...
        return Card(
          ...
          children: byClass.entries.map((classEntry) {
            ...
            rows.sort(
                (a, b) => '${b['lessonDate']}'.compareTo('${a['lessonDate']}'));
