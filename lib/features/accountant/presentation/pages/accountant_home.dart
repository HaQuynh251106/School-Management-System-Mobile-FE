...
/// Công cụ tài chính của Admin. Đây là một năng lực quản trị, không phải một
/// không gian đăng nhập hay vai trò sản phẩm riêng.
...
class _FinanceOperationsPageState extends State<FinanceOperationsPage> {
  ...
  void _refresh() => setState(() => _dataRevision++);
  ...
  Widget build(BuildContext context) => DefaultTabController(
    ...
    child: Scaffold(
      appBar: AppBar(
        title: const Text('Tài chính và đối soát'),
        actions: [
          IconButton(
            tooltip: 'Tải lại',
            onPressed: _refresh,
...
class _FinanceOverviewPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _FinanceFuture<_FinanceData>(
    ...
    builder: (context, data, reload) {
      final total = data.invoices.fold<num>(
        0,
        (sum, item) => sum + _number(item['totalAmount']),
      );
      final paid = data.invoices.fold<num>(
        0,
        (sum, item) => sum + _number(item['paidAmount']),
      );
      final overdue = data.invoices
      ...
      return RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            const RolePageIntro(
              title: 'Tài chính nhà trường',
              subtitle:
                  'Theo dõi đợt thu, công nợ và các giao dịch đang chờ xác nhận.',
              accent: AppColors.accountantAccent,
              icon: Icons.account_balance_wallet_rounded,
              badges: ['VietQR', 'Đối soát giao dịch'],
            ),
            GridView.count(
            ...
            Card(
              child: ListTile(
                ...
                title: Text('${data.pending.length} giao dịch VietQR đang chờ'),
                subtitle: const Text(
                        ...
                        child: FilledButton.tonalIcon(
                          onPressed: () async {
                            final invoices = await sl<ApiService>()
                                .generateInvoices(period['id'].toString());
                            if (context.mounted) {
...
class _DebtPageState extends State<_DebtPage> {
  ...
  Future<void> _recordCash(Map<String, dynamic> invoice) async {
    ...
    final payerController = TextEditingController(
      text: (invoice['parentName'] ?? '').toString(),
    );
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Người nộp tiền'),
              ),
              const SizedBox(height: 12),
              TextField(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            apiErrorMessage(
              error,
              fallback: 'Không thể ghi nhận khoản thu. Vui lòng thử lại.',
            ),
          ),
        ),
      );
  ...
  Future<void> _refund(Map<String, dynamic> invoice) async {
    ...
    if (result == null) return;
    try {
      await sl<ApiService>().refundInvoice(
        invoice['id'].toString(),
        result.$1,
        result.$2,
      );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Đã hoàn ${_money(result.$1)}')));
      _reload();
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                (invoice['studentName'] ?? invoice['code'] ?? '').toString(),
                style: Theme.of(
                  context,
                ).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800),
              ),
              ...
              if (!canCollect && !canRefund)
                const Text(
                  'Hóa đơn không có thao tác tài chính khả dụng.',
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      leading: CircleAvatar(
                        child: Text(
                          (invoice['classCode'] ?? '?').toString().replaceAll(
                            'Lớp ',
                            '',
                          ),
                        ),
                      ...
                      title: Text(
                        (invoice['studentName'] ?? invoice['code'] ?? '')
                            .toString(),
                      ),
                      subtitle: Text(
          ...
          children: [
            ...
            ...items.map((raw) {
              ...
              final invoice = raw['invoice'] is Map
                  ...
                  : <String, dynamic>{};
              final id = (payment['id'] ?? raw['paymentId'] ?? '').toString();
              return Card(
                child: Padding(
                  padding: const EdgeInsets.all(14),
                  child: Column(
